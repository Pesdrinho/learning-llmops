...
class CostLimiter:
    ...
    def __init__(self, daily_limit_usd: float = 15.0):
        """
        ...
        Exemplo:
            >>> limiter = CostLimiter(daily_limit_usd=20.0)
        """
        self.daily_limit_usd = daily_limit_usd
        self.model_prices = self._load_model_prices()
        logger.info(f"CostLimiter inicializado com limite diário de US${daily_limit_usd}")
    ...
    def _load_model_prices(self) -> dict:
        ...
        try:
            ...
            # Fallback: preços padrão para não quebrar a API
            return {
                "gpt-oss-120b": {
                    "in_usd_per_1k": 0.10,
                    "out_usd_per_1k": 0.20
    ...
    def calculate_cost(
        ...
        Args:
            model: Identificador do modelo (ex: "gpt-oss-120b")
    ...
    async def get_daily_spend(
        ...
        try:
            async with db.pool.acquire() as conn:
                result = await conn.fetchrow(query, api_key_hash)
                total = float(result['total']) if result else 0.0

                logger.debug(f"Gasto diário para {api_key_hash[:8]}...: ${total:.4f}")
                return total
    ...
    async def check_limit(
        ...
        Lógica:
        1. Busca gasto atual do dia
        2. Soma com custo estimado desta requisição
        3. Compara com limite diário
        ...
        """
        # Obtém gasto atual
        current_spend = await self.get_daily_spend(api_key, db)

        # Calcula gasto projetado
        ...
        if projected_spend >= self.daily_limit_usd:
            message = (
                ...
                f"Limite: ${self.daily_limit_usd:.2f}. "
        ...
        message = f"OK. Gasto atual: ${current_spend:.2f}, Restante: ${remaining:.2f}"
    ...
    async def record_spend(
        ...
        try:
            ...
            logger.info(
                f"Gasto registrado: ${cost_usd:.6f} | "
                f"Modelo: {model} | "
                f"Hash: {api_key_hash[:8]}..."
            )
...
async def cost_limit_middleware(request: Request, call_next):
    ...
    if request.url.path.startswith("/chat"):
        try:
            ...
            logger.error(f"Erro no middleware de custo: {e}")
