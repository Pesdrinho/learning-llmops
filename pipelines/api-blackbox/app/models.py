...
class ChatMessage(BaseModel):
    ...
    name: str | None = Field(
        None,
        description="Nome do participante (opcional, usado em multi-agentes)"
    )
...
class ChatRequest(BaseModel):
    ...
    model: str = Field(
        "gpt-oss-120b",
        description="Identificador do modelo LLM (ex: gpt-oss-120b, anthropic/claude-3.5-sonnet)",
        examples=["gpt-oss-120b", "anthropic/claude-3.5-sonnet"]
    )
    ...
    top_p: float = Field(
        1.0,
        description="Nucleus sampling (0.0 a 1.0). Alternativa à temperature",
        ge=0.0,
        le=1.0
    )
    ...
    stream: bool = Field(
        False,
        description="Se True, retorna resposta em streaming (Server-Sent Events)"
    )

    user_id: str | None = Field(
        None,
        description="ID do usuário (para tracking e rate limiting)"
    )
...
class UsageInfo(BaseModel):
    ...
    prompt_tokens: int = Field(
        ...,
        description="Número de tokens na entrada (mensagens enviadas)"
    )

    completion_tokens: int = Field(
        ...,
        description="Número de tokens na saída (resposta gerada)"
    )

    total_tokens: int = Field(
        ...,
        description="Total de tokens processados (prompt + completion)"
    )
...
class ChatResponse(BaseModel):
    ...
    usage: UsageInfo = Field(
        ...,
        description="Informações de consumo de tokens"
    )

    cost_usd: float = Field(
        ...,
        description="Custo desta requisição em dólares",
        ge=0.0
    )
    ...
    request_id: str = Field(
        ...,
        description="ID único da requisição (para debugging)"
    )
...
class ModelInfo(BaseModel):
    ...
    provider: str = Field(
        ...,
        description="Provider do modelo (ex: OpenAI, Anthropic)"
    )

    input_cost_per_1k: float = Field(
        ...,
        description="Custo por 1k tokens de entrada (USD)",
        ge=0.0
    )

    output_cost_per_1k: float = Field(
        ...,
        description="Custo por 1k tokens de saída (USD)",
        ge=0.0
    )

    context_window: int = Field(
        ...,
        description="Tamanho máximo do contexto (tokens)",
        gt=0
    )
...
class ModelsResponse(BaseModel):
    ...
    total: int = Field(
        ...,
        description="Total de modelos disponíveis"
    )
...
class HealthResponse(BaseModel):
    ...
    checks: dict = Field(
        ...,
        description="Status de dependências (database, openrouter, etc)"
    )
...
class ErrorDetail(BaseModel):
    ...
    request_id: str | None = Field(
        None,
        description="ID da requisição que causou o erro"
    )
